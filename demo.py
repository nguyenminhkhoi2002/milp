...
def supply_rule(model, i):
    return (
        sum(model.x[i, j] for j in model.M) <= 100
    )  # Giả sử mỗi nguồn cung cấp tối đa 100 đơn vị hàng hóa
...
def demand_rule(model, j):
    return (
        sum(model.x[i, j] for i in model.N) >= 50
    )  # Giả sử mỗi đích cần tối thiểu 50 đơn vị hàng hóa


model.demand_constraint = Constraint(model.M, rule=demand_rule)

# Giải mô hình
solver = SolverFactory("cpc")
solver.solve(model)
...
print(
    f"Chi phí vận chuyển tối thiểu: {model.objective()}"
)  # In giá trị của hàm mục tiêu
